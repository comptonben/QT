...
void CheckSliderSum(Dialog& w)
{
    ...
    for(int k = 0; k <= 5; k++)
    {
        ...
        if ( w.ui->rightDial->value() + w.ui->rightDial->value() != 5)
        {
            passed = false;
            break;
        }
...
void CheckInitialization(Dialog& w)
{
    ...
    if ( IntWidgetCheck(QString("leftSlider->value()"), 0, w.ui->leftDial->value()) )
    {
        score = score + delta;
    }

    if ( IntWidgetCheck(QString("rightSlider->value()"), 5, w.ui->rightDial->value()) )
    {
        score = score + delta;
    }
...
void SaveOriginalInitialization(Dialog& w)
{
    ...
    lSliderValue = w.ui->leftDial->value();
    rSliderValue = w.ui->rightDial->value();
...
void RestoreOriginalInitialization(Dialog& w)
{
    ...
    w.ui->leftDial->setValue(lSliderValue);
    w.ui->rightDial->setValue(rSliderValue);
